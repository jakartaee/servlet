...
parsed from the `String` value returned by the `getRequestURI` method.
...
. The HTTP method is POST or QUERY.
...
Be aware that any filter that modifies the `ServletResponse` can potentially
break a subsequent filter or the servlet in the `FilterChain`. For example, if
the servlet supports range requests or sets a content length, a filter that
modifies the response body is likely to break the servlet. It is the
responsibility of the filter developer to clearly describe what it does with the
`ServletResponse` so a user of the filter is aware of what happens when they add
the filter.

Filters that modify the response body generated by a servlet should take steps
to ensure that they minimise the risk of breaking a subsequent filter or the
servlet in the `FilterChain`. As as minimum, filters that modify the response
body generated by a servlet where the `Content-Length` header is set by point
the servlet's `service()` method exits, must ensure that the response has an
appropriate `Content-Length` header either by ensuring that the correct value is
set for the `Content-Length` header set or by ensuring that the `Content-Length`
is not set. 
...
For each `<welcome-file>` or legacy `<welcome-file>` in the order it appears in
the deployment descriptor:
- append the partial URI to the request URI to create a welcome URI
- if a static file exists in the web application for the welcome URI, map the
  welcome URI using the standard servlet mapping rules and process with the
  mapped servlet
- if the welcome URI is an exact match to a mapping for a servlet, process with
  that servlet

For each `<welcome-servlet>` or legacy `<welcome-file>` in the order it appears
in the deployment descriptor:
- append the partial URI to the request URI to create a welcome URI
- map the welcome URI to a servlet using the standing mapping rules
- if the welcome URI maps to a servlet other than the default servlet, process
  with that servlet
...
only be used to authenticate as the user at a specific realm.]
 be available to the authenticating
...
request or until the application successfully calls `authenticate`,
...
written to comply with Servlet 4.0 onwards must still be supported such that applications continue to deploy.
Servlet containers may also optionally support deployment descriptors written to comply with earlier specification versions.  
For the actual XSD files please visit `https://jakarta.ee/xml/ns/jakartaee/`.
...
identity of a web user to be propagated to the Jakarta Enterprise Bean
container.
...
using the `@DeclareRoles` annotation. The `@DeclareRoles` annotation may
...
`myCart` prior to the class declaring the injection being made
available.
...
link:https://github.com/jakartaee/servlet/issues/11[Issue 11]::
Clarify the behaviour of `ServletContext.getRealPath()` when the virtual path
can be mapped to a physical path but the physical path does not exist.
...
to minimise the risk of any breakage and require a minimum level of action by
such filters to mitigate the biggest risk.
...
link:https://github.com/jakartaee/servlet/issues/661[Issue 661]::
Limit the requirement on `PushBuilder` to remove cookies to those `Cookie` instances where
`Cookie.getMaxAge()` returns zero. This allows `Cookie` instances where `Cookie.getMaxAge()`
returns a negative value to be retained which is consistent with (per RFC 6265) such cookies
being retained until "the current session is over" as defined by the user agent.  
...
link:https://github.com/jakartaee/servlet/issues/981[Issue 981]::
Add constants to `HttpServletResponse` for the status codes defined in RFC 6585.

link:https://github.com/jakartaee/servlet/issues/994[Issue 994]::
The value returned by `HttpServletRequest.getpathInfo()` does not contain path paremeter information.

link:https://github.com/jakartaee/servlet/issues/1068[Issue 1068]::
...
Remove the restriction on programmatically added listeners calling some `ServletContext` getter methods.
